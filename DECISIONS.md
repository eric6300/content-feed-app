...
## 2026-09-16 — Hardening service artwork and image failure states

**Considered:** Keep the 20 generic solid-color/circle SVGs and text-only image fallback, vs. introduce remote or bitmap artwork and a larger placeholder treatment.
**Decided:** Replace the bundled artwork with 20 local 320 × 144 vector product plates using the existing Signal Desk palette and distinct geometric silhouettes. Keep image handling deterministic and offline-safe, but make the shared placeholder responsive: Material image/broken-image icons always show, while the state label appears only when the surface is wide enough. Feed article titles and summaries are capped at two and three lines respectively with ellipsis, and the unkept bookmark uses the semantic `onSurfaceVariant` tint so it remains visible in dark mode.
**Trade-off:** The vector set adds authored asset maintenance and is less literal than photography, but it avoids another network dependency, matches the service-card aspect ratio, stays small, and prevents missing/long content from breaking feed scanability.
