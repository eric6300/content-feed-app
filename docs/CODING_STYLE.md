...
- Module direction is one-way: `:app` composes `:feed`, `:core`, and `:designsystem`; `:feed` consumes `:core` and `:designsystem`; `:designsystem` depends on no project module. Tokens and generic UI primitives belong in `:designsystem`, while product-specific screen composition stays in `:feed` or `:app`.
