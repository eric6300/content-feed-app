...
**Out of scope** (not included in this release; see `README.md` for current limitations and verification gaps):
- Search / filtering
...
- **Build compatibility**: T1 established the baseline and T6 upgraded it for Navigation 3. The project uses AGP `8.9.1`, Kotlin `2.2.10`, Gradle `8.13`, compileSdk `36`, targetSdk `35`, and minSdk `24`. `targetSdk` was intentionally not raised with `compileSdk`: the Android 16 behavior changes that come with `targetSdk 36` have not been verified on device. All external versions are centralized in `gradle/libs.versions.toml` and selected as compatible with the committed build.
...
| Open-Meteo | Current conditions + daily forecast for the fixed display location `25.0330, 121.5654` | Live, no key; no location permission |
| Bundled service-card catalog (seeded from DummyJSON) | Product title, thumbnail (used as the card's icon), a short blurb (truncated from the product description), price, and a synthesized target-action link | Local static JSON and 20 bundled product images; no runtime DummyJSON dependency |
...
| All UI states handled explicitly: loading, empty, error, offline, back-online, pagination-end, and pagination-error | Feature: Loading, empty, and error states; Feature: Per-source data handling; Feature: Connectivity (offline/back-online banners and external-link actions offline) |
| Android behavior and accessibility constraints | `DESIGN.md` → Adaptation; `DESIGN_TOKENS.md` → App shell and Implementation/review gates; `IMPLEMENTATION_PLAN.md` → T7 verification gate |
